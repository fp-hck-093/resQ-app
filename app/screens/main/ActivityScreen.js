...
const STATUS_CONFIG = {
  pending: { color: '#f97316', bg: '#fff7ed', icon: 'time-outline', label: 'Pending' },
  in_progress: { color: '#3b5fca', bg: '#eff6ff', icon: 'reload-outline', label: 'In Progress' },
  completed: { color: '#22c55e', bg: '#f0fdf4', icon: 'checkmark-circle-outline', label: 'Selesai' },
  active: { color: '#3b5fca', bg: '#eff6ff', icon: 'reload-outline', label: 'Aktif' },
  cancelled: { color: '#ef4444', bg: '#fef2f2', icon: 'close-circle-outline', label: 'Dibatalkan' },
};
...
export default function ActivityScreen() {
  ...
  const { data: activitiesData, loading: activitiesLoading, refetch: refetchActivities } = useQuery(GET_MY_ACTIVITIES);
  const { data: requestsData, loading: requestsLoading, refetch: refetchRequests } = useQuery(GET_MY_REQUESTS);

  const [updateStatus, { loading: updateLoading }] = useMutation(UPDATE_ACTIVITY_STATUS, {
      ...
      <View style={styles.card}>
        <View style={styles.cardInner}>
          ...
          <View style={styles.cardBody}>
            <Text style={styles.cardTitle}>Request #{item.requestId?.slice(-6).toUpperCase()}</Text>
        ...
        {item.status === 'active' && (
          <View style={styles.actionRow}>
            <TouchableOpacity
              ...
              onPress={() => updateStatus({ variables: { requestId: item.requestId, status: 'completed' } })}
            ...
            <TouchableOpacity
              ...
              onPress={() => updateStatus({ variables: { requestId: item.requestId, status: 'cancelled' } })}
              disabled={updateLoading}
            >
  ...
  const renderRequest = ({ item }) => {
    const catConfig = CATEGORY_CONFIG[item.category] || { color: '#6b7280', bg: '#f9fafb', icon: 'help-circle' };
    const statusConfig = STATUS_CONFIG[item.status] || STATUS_CONFIG.pending;
    return (
      <View style={styles.card}>
        <View style={styles.cardInner}>
          <View style={[styles.cardIconWrap, { backgroundColor: catConfig.bg }]}>
        ...
        <View style={styles.cardFooter}>
          ...
          <View style={styles.cardMetaItem}>
            ...
            <Text style={styles.cardMetaText} numberOfLines={1}>{item.address || 'Lokasi tidak tersedia'}</Text>
          </View>
        </View>
      </View>
    );
  ...
  const isLoading = activeTab === 'volunteer' ? activitiesLoading : requestsLoading;
  const isEmpty = activeTab === 'volunteer' ? activities.length === 0 : myRequests.length === 0;
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.tabContainer}>
        ...
        >
          ...
          {activities.length > 0 && (
            <View style={[styles.tabBadge, activeTab === 'volunteer' && styles.tabBadgeActive]}>
              <Text style={[styles.tabBadgeText, activeTab === 'volunteer' && styles.tabBadgeTextActive]}>
                {activities.length}
              </Text>
            </View>
          )}
        ...
        >
          ...
          />
          <Text style={[styles.tabText, activeTab === 'victim' && styles.tabTextActive]}>
            Request Saya
          </Text>
          {myRequests.length > 0 && (
            <View style={[styles.tabBadge, activeTab === 'victim' && styles.tabBadgeActive]}>
              <Text style={[styles.tabBadgeText, activeTab === 'victim' && styles.tabBadgeTextActive]}>
                {myRequests.length}
              </Text>
            </View>
          )}
      ...
      {isLoading ? (
        ...
        <View style={styles.emptyContainer}>
          <View style={styles.emptyIconWrap}>
            <Ionicons
              name={activeTab === 'volunteer' ? 'hand-left-outline' : 'alert-circle-outline'}
              size={48}
              color="#3b5fca"
            />
          </View>
          <Text style={styles.emptyTitle}>
            {activeTab === 'volunteer' ? 'Belum Ada Aktivitas' : 'Belum Ada Request'}
          </Text>
        ...
        <FlatList
          ...
          renderItem={activeTab === 'volunteer' ? renderActivity : renderRequest}
          contentContainerStyle={styles.listContent}
          showsVerticalScrollIndicator={false}
          refreshControl={
            <RefreshControl
              refreshing={isLoading}
              onRefresh={activeTab === 'volunteer' ? refetchActivities : refetchRequests}
              colors={['#3b5fca']}
            />
          }
        />
...
const styles = StyleSheet.create({
  ...
  headerSubtitle: { fontSize: 13, color: 'rgba(255,255,255,0.7)', marginTop: 2, marginBottom: 16 },
  ...
  tabActive: { backgroundColor: '#eff6ff', borderWidth: 1.5, borderColor: '#3b5fca' },
